...
export const OAUTH_AVAILABLE =
  process.env.NEXT_PUBLIC_ENABLE_GOOGLE_OAUTH === "true" ||
  process.env.NEXT_PUBLIC_ENABLE_LINKEDIN_OAUTH === "true";
...
export function OAuthButtons() {
  ...
  const linkedinEnabled = process.env.NEXT_PUBLIC_ENABLE_LINKEDIN_OAUTH === "true";

  if (!OAUTH_AVAILABLE) return null;
  ...
  return (
    <>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,130px),1fr))", gap: 12 }}>
        {linkedinEnabled && <button type="button" onClick={() => go("linkedin_oidc")} style={oauthBtn}>
        ...
        {googleEnabled && <button type="button" onClick={() => go("google")} style={oauthBtn}>
