...
function Feature({ icon, children }: { icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
      <span style={{ display: "grid", placeItems: "center", width: 34, height: 34, flex: "none", borderRadius: 9, background: "rgba(232, 138, 161,.16)", color: "var(--teal-light)" }}>{icon}</span>
      <span style={{ fontSize: 14, color: "rgba(255,255,255,.82)" }}>{children}</span>
    </div>
  );
}
    ...
    <AuthSplit
      panel={
        <>
          ...
          <p style={{ color: "rgba(255,255,255,.70)", lineHeight: 1.9, fontSize: 14.5, marginBlockEnd: 26 }}>{t("panelSignupBody")}</p>
