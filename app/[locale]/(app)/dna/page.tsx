...
export default async function DnaPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const tone = split(t("tone"));
  const hooks = split(t("hooks"));
  ...
  return (
    <main style={{ maxWidth: 860, margin: "0 auto", padding: "clamp(24px,4vw,40px) clamp(16px,4vw,32px) 80px", animation: "floatUp .4s ease" }}>
      ...
      <section style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "var(--r-lg)", padding: "clamp(16px,3vw,24px)", marginBlockStart: 20 }}>
        <p style={{ fontSize: 14.5, color: "var(--slate)", lineHeight: 1.9, marginBlockEnd: 18 }}>{t("summary")}</p>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 16 }}>
          <Field label={t("dialectLabel")}>
            <span style={{ display: "inline-block", padding: "5px 12px", borderRadius: 999, background: "var(--teal-tint)", color: "var(--teal-deep)", fontSize: 13, fontWeight: 600 }}>{t("dialect")}</span>
          </Field>
          <Field label={t("audienceLabel")}>
            <span style={{ fontSize: 14, color: "var(--slate)" }}>{t("audience")}</span>
          </Field>
        </div>

        <Field label={t("toneLabel")}>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
            {tone.map((x, i) => <span key={i} style={{ padding: "5px 11px", borderRadius: 999, background: "var(--gold-tint)", color: "var(--gold-dark)", fontSize: 12.5, fontWeight: 600 }}>{x}</span>)}
          </div>
        </Field>

        <Field label={t("hooksLabel")}>
          <ul style={{ margin: 0, paddingInlineStart: 18, color: "var(--slate)", fontSize: 14, lineHeight: 2 }}>
            {hooks.map((x, i) => <li key={i}>{x}</li>)}
          </ul>
        </Field>
        ...
        <Field label={t("completionLabel")}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ flex: 1, height: 8, background: "var(--border)", borderRadius: 8, overflow: "hidden" }}>
              <div style={{ width: "82%", height: "100%", background: "linear-gradient(90deg,var(--teal),var(--teal-dark))" }} />
            </div>
            <span style={{ fontSize: 13, fontWeight: 700, color: "var(--teal-deep)" }}>٨٢٪</span>
          </div>
        </Field>
      </section>
    </main>
